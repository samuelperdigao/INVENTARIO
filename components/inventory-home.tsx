  ...
  return (
    <main className="shell dashboard-shell">
      <header className="dashboard-topbar">
        ...
        <div className="account-actions"><span className="user-chip"><span>{firstName(user.displayName).slice(0, 1).toUpperCase()}</span>{user.displayName}</span><button className="text-button" type="button" onClick={() => void signOut()}>Sair</button></div>
      </header>
      ...
      {user.teams.length === 0 ? <p className="notice">Sua conta ainda não está associada a uma equipe. Você pode participar de um inventário pelo código; a primeira sincronização de um inventário novo exige associação administrativa.</p> : null}
