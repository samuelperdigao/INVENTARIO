...
it("formata OK com um local sem repetir quantidade", () => {
  expect(formatLotLocation(location("DE", "15", 20), "OK")).toBe("LP 15");

  const list = renderLocations("OK", [location("DE", "15", 20)]);
  expect(list).toHaveTextContent("LP 15");
  expect(list).not.toHaveTextContent("20");
  expect(list).not.toHaveTextContent("pç");
  expect(list).not.toHaveTextContent(/[()]/);
});

it("formata OK no lado LE sem repetir quantidade", () => {
  expect(formatLotLocation(location("EF", "3", 3), "OK")).toBe("LE 3");

  const list = renderLocations("OK", [location("EF", "3", 3)]);
  expect(within(list).getByRole("listitem", { name: "LE 3" })).toBeInTheDocument();
  expect(list).not.toHaveTextContent("3 pç");
});

it("mantém cada localização e sua quantidade em uma linha para peça solteira", () => {
  expect(formatLotLocation(location("EF", "15", 19), "PEÇA_SOLTEIRA")).toBe("LE 15 · 19 pç");

  const list = renderLocations("PEÇA_SOLTEIRA", [location("EF", "15", 19), location("DE", "21", 1)]);
  const rows = within(list).getAllByRole("listitem");
  expect(rows).toHaveLength(2);
  expect(rows.map((row) => row.getAttribute("aria-label"))).toEqual(["LE 15 · 19 pç", "LP 21 · 1 pç"]);
  expect(rows[0].querySelector(".lot-location-name")).toHaveTextContent("LE 15");
  expect(rows[0].querySelector(".lot-location-quantity")).toHaveTextContent("19 pç");
  expect(rows[1].querySelector(".lot-location-name")).toHaveTextContent("LP 21");
...
it("preserva vãos de um e dois dígitos junto do lado", () => {
  ...
  expect(within(list).getByRole("listitem", { name: "LP 1 · 8 pç" })).toBeInTheDocument();
  expect(within(list).getByRole("listitem", { name: "LE 10 · 12 pç" })).toBeInTheDocument();
});
...
it("mantém nome e quantidade em elementos não quebráveis para mobile", () => {
  ...
  expect(row).toHaveAttribute("aria-label", "LE 15 · 19 pç");
});
...
it("não repete quantidade física para lotes OK mesmo com classificação sem acento visual", () => {
  ...
  expect(list).toHaveTextContent("LP 2");
...
it("mantém camada no local apresentado pelo backend", () => {
  const presentation: LotPresentation = {
    ...
    primaryLocation: { label: "LP 15 · A1", display: "LP 15 · A1 · 19 pç", quantity: 19, isPrimary: true },
    otherLocations: [{ label: "LP 15 · A2", display: "LP 15 · A2 · 1 pç", quantity: 1, isPrimary: false }],
    locations: [
      { label: "LP 15 · A1", display: "LP 15 · A1 · 19 pç", quantity: 19, isPrimary: true },
      { label: "LP 15 · A2", display: "LP 15 · A2 · 1 pç", quantity: 1, isPrimary: false },
    ],
    outOfPrimaryQuantity: 1,
    action: "Conferir a peça localizada em LP 15 · A2.",
  };
  ...
  expect(within(list).getByRole("listitem", { name: "LP 15 · A1 · 19 pç" })).toBeInTheDocument();
  expect(within(list).getByRole("listitem", { name: "LP 15 · A2 · 1 pç" })).toBeInTheDocument();
});
