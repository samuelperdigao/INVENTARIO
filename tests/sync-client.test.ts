...
function response(inventoryId: string, entryId: string, revision = 1) {
  ...
  return {
    ...
    entries: [{ id: entryId, inventoryId, side: "EF", bay: "01", layer: "A1", lot: "000123", quantity: 3, duplicateConfirmed: false, createdAt: timestamp, updatedAt: timestamp, revision, syncBaseRevision: revision, tombstone: false, deletedAt: null }],
...
it("confirma alterações locais somente após a resposta idempotente do servidor", async () => {
  ...
  const entry = await createEntry(inventory.id, { side: "EF", bay: "01", layer: "A1", lot: "000123", quantity: 3 });
...
it("mantém as duas versões quando o servidor reporta um conflito", async () => {
  ...
  server.conflicts = [{
    ...
    entityId: entry.id,
    serverRecord: { id: entry.id, inventoryId: inventory.id, side: "DE", bay: "02", layer: "A2", lot: "200", quantity: 5, duplicateConfirmed: false, createdAt: "2026-09-11T12:00:00.000Z", updatedAt: "2026-09-11T12:00:00.000Z", revision: 2, syncBaseRevision: 2, tombstone: false, deletedAt: null },
  }];
  ...
  expect(conflicts[0].localRecord).toMatchObject({ lot: "100", layer: "A1" });
  expect(conflicts[0].serverRecord).toMatchObject({ lot: "200", layer: "A2" });
  expect((await db.entries.get(entry.id))?.syncStatus).toBe("ERROR");

  await resolveConflict(inventory.id, conflicts[0].id, "local");
  expect((await db.entries.get(entry.id))).toMatchObject({ lot: "100", layer: "A1", revision: 3, syncBaseRevision: 2, syncStatus: "PENDING" });
});
